...
export default function CartOrderDone({ orderId, cdekTrack, buyerEmail, isLoggedIn, onGoToAuth }: CartOrderDoneProps) {
  return (
      ...
      {!isLoggedIn && onGoToAuth && (
        <div className="bg-card border border-border rounded-2xl p-5 mb-4 text-left">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-semibold text-foreground">Войдите, чтобы отслеживать заказы</span>
          </div>
          <p className="text-xs text-muted-foreground mb-4">
            В личном кабинете вы всегда сможете посмотреть статус и историю заказов.
          </p>
          <div className="flex flex-col gap-2">
            <button
              onClick={() => onGoToAuth(buyerEmail)}
